...
namespace triqs::modest {
  ...
  std::optional<nda::array<dcomplex, 5>> load_rotate_and_format_delta_projectors(std::string const &filename, ReadMode mode,
    ...
    auto delta_P_k_list =
       range(n_atoms) | stdv::transform([&](auto atom) { return delta_P_k_tmp(r_all, r_all, r_all, atom, r_all, r_all); }) | tl::to<std::vector>();
    ...
    for (auto const &[atom, sli] : enumerated_sub_slices(atom_decomp)) {
      delta_P_k(r_all, r_all, r_all, sli, r_all) = delta_P_k_list[atom](r_all, r_all, r_all, nda::range(0, atom_decomp[atom]), r_all);
    }
  ...
  std::pair<double, one_body_elements_on_grid> read_obe_from_dft_converter_hdf5(std::string const &filename, double threshold,
    ...
    auto proj =
       downfolding_projector{.spin_kind = spin_kind, .P_k = std::move(P_k), .delta_P_k = std::move(delta_P_k), .n_bands_per_k = n_bands_per_k};
  ...
  one_body_elements_on_grid read_partial_projectors_for_obe(const std::string &filename, const one_body_elements_on_grid &obe) {
    ...
    auto partial_proj = downfolding_projector{
       .spin_kind = obe.C_space.spin_kind(), .P_k = std::move(P_k), .delta_P_k = std::nullopt, .n_bands_per_k = obe.H.n_bands_per_k};
  ...
  one_body_elements_on_grid read_data_on_high_symm_path_for_obe(const std::string &filename, const one_body_elements_on_grid &obe) {
    ...
    auto proj = downfolding_projector{
       .spin_kind = obe.C_space.spin_kind(), .P_k = std::move(P_k), .delta_P_k = std::nullopt, .n_bands_per_k = n_bands_per_k};
