...
namespace triqs::modest {
  ...
  one_body_elements_on_grid permute_local_space(std::vector<std::vector<long>> const &atom_partition, one_body_elements_on_grid const &obe) {
    ...
    if (obe.P.delta_P_k.has_value()) {
      ...
      auto delta_P_k_list =
         atom_decomp | stdv::transform([&](auto dim) { return nda::zeros<dcomplex>(n_k, n_sig, n_delta, dim, n_nu); }) | tl::to<std::vector>();

      for (auto &&[atom, sli] : enumerated_sub_slices(atom_decomp)) {
        delta_P_k_list[atom](r_all, r_all, r_all, r_all, r_all) = obe.P.delta_P_k.value()(r_all, r_all, r_all, sli, r_all);
      }
      ...
      for (auto const &[atom, sli] : enumerated_sub_slices(new_atom_decomp)) {
        new_delta_P_k(r_all, r_all, r_all, sli, r_all) = new_delta_P_k_list[atom](r_all, r_all, r_all, nda::range(0, new_atom_decomp[atom]), r_all);
      }
